...
public class EconomyCommand {
    ...
    public void register() {
        ...
        new CommandAPICommand("economy")
            .withAliases("eco", "econ")
            .withPermission("mangoeconomy.economy.admin")
            .withSubcommands(
                    ...
                    .executes((sender, args) -> {
                        ...
                        if (economyManager.addMoney(target, amount)) {
                            ...
                            MessageUtils.sendMessage(target, 
                                ...
                                economyManager.formatMoney(economyManager.getBalance(target)) + "</yellow>");
                        } else {
                ...
                new CommandAPICommand("take")
                    ...
                    .executes((sender, args) -> {
                        ...
                        if (economyManager.removeMoney(target, amount)) {
                            ...
                            MessageUtils.sendMessage(sender, 
                                "<red>Error: <aqua>" + target.getName() + "</aqua> no tiene suficiente dinero. " +
                                "Saldo actual: <yellow>" + economyManager.formatMoney(economyManager.getBalance(target)) + "</yellow>");
                        }
                    }),
                ...
                new CommandAPICommand("set")
                    ...
                    .executes((sender, args) -> {
                        ...
                        MessageUtils.sendMessage(target, 
                            "<green>Tu saldo ha sido establecido a <yellow>" + economyManager.formatMoney(amount) + 
                            "</yellow> monedas. (Anterior: <gray>" + economyManager.formatMoney(previousBalance) + "</gray>)");
                    }),
                ...
                new CommandAPICommand("reload")
                    .executes((sender, args) -> {
                        ...
                        MessageUtils.sendMessage(sender, "<green>Configuración y datos de economía recargados correctamente.");
                    }),
                ...
                new CommandAPICommand("top")
                    ...
                    .withSubcommand(
                        new CommandAPICommand("amount")
                            .withArguments(new dev.jorel.commandapi.arguments.IntegerArgument("limit", 1, 50))
                            .executes((sender, args) -> {
                                Object limitObj = args.get("limit");
                                if (limitObj == null) {
                                    MessageUtils.sendMessage(sender, "<red>Error: Límite no válido.");
                                    return;
                                }
                ...
                new CommandAPICommand("stats")
                    .executes((sender, args) -> {
                        EconomyManager economyManager = EconomyManager.getInstance();
                        com.spectrasonic.MangoEconomy.Main plugin = (com.spectrasonic.MangoEconomy.Main) 
                            org.bukkit.Bukkit.getPluginManager().getPlugin("MangoEconomy");
                        ...
                        MessageUtils.sendMessage(sender, "<green>🏦 Saldo inicial: <yellow>" + 
                            economyManager.formatMoney(economyManager.getStartingBalance()) + "</yellow>");
                        MessageUtils.sendMessage(sender, "<green>💎 Moneda: <yellow>" + 
                            economyManager.getCurrencyName() + " (" + economyManager.getCurrencySymbol() + ")</yellow>");
                        
                        // Estado de Vault
                        if (plugin != null && plugin.isVaultEnabled()) {
                            MessageUtils.sendMessage(sender, "<green>✅ Vault: <yellow>HABILITADO</yellow>");
                            MessageUtils.sendMessage(sender, "<green>   Otros plugins pueden usar MangoEconomy</yellow>");
                        } else {
                            MessageUtils.sendMessage(sender, "<red>❌ Vault: <yellow>DESHABILITADO</yellow>");
                        }
                    }),
    ...
    private void showTopBalances(org.bukkit.command.CommandSender sender, int limit) {
        ...
        for (java.util.Map.Entry<String, Double> entry : topBalances.entrySet()) {
            ...
            switch (position) {
                ...
                case 2: medal = "🥈"; break;
                case 3: medal = "🥉"; break;
                default: medal = "<gray>" + position + ".</gray>"; break;
            }
